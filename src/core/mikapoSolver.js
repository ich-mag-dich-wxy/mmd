...
export class Solver {
  ...
  // three.js 适配：从普通 {x,y,z} 对象构造 Babylon Vector3 后再 calibrate。
  // 调用方负责把 three.js 世界坐标按需翻轴后再传入（mikapo.js 翻 Z）。
  calibrateFromPlain(plainPos) {
    const vec3Map = {}
    for (const name of Object.keys(plainPos)) {
      const p = plainPos[name]
      vec3Map[name] = new Vector3(p.x, p.y, p.z)
    }
    this.calibrate(vec3Map)
  }
